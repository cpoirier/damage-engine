<?php if (defined($inc = "SQL_EXPRESSION_TOKENIZER_INCLUDED")) { return; } else { define($inc, true); }
...
// =============================================================================================
  ...
  class SQLExpressionTokenizer
  {
    ...
    static function &tokenize( $expression, $strip_whitespace = false )
    {
      ...
      if( $strip_whitespace )
      {
        ...
        {
          if( $token->type != static::WHITESPACE )
          {
            $tokens[] = $token;
          }
        }
    ...
    protected function next_token()
    {
      ...
      $type = null;
      while( !$type && $this->pos < $this->length )
      {
        $c = substr($this->raw, $this->pos++, 1);
        ...
        switch( $c )
        {
          case "'":
            $type = static::STRING;
            while( $this->pos < $this->length && !is_null($c = substr($this->raw, $this->pos++, 1)) )
            {
              print $c;
              if( $c == '\\' )
              {
          ...
          case "9":
            $type = static::NUMBER;
            while( $this->pos < $this->length && ($c = substr($this->raw, $this->pos, 1)) >= "0" && $c <= "9" )
            {
              print $c;
              $this->pos++;
            }
          ...
          case '\n':
            ...
            while( $this->pos < $this->length && (($c = substr($this->raw, $this->pos, 1)) == ' ' || $c == '\t' || $c == '\r' || $c == '\n') )
            {
              print $c;
              $this->pos++;
            }
          ...
          case '`':
            ...
            while( $this->pos < $this->length && ($c = substr($this->raw, $this->pos++, 1)) != '`' )
            {
              print $c;
            }
            break;
          ...
          default:
            $type = static::WORD;
            while( $this->pos < $this->length && ($c = substr($this->raw, $this->pos, 1)) && preg_match('/[\w\d_]/', $c) )
            {
              print $c;
              $this->pos++;
            }
